...
import type { Transport, AgentSession, AgentRequest, AgentResponse } from '../../transport/transport';
...
describe('AgentExecutor retry', () => {
  ...
  test('重试成功：前两次失败、第三次成功 → COMPLETED', async () => {
    ...
    customTransport.connect = async function (agentId: string) {
      attempt++;
      if (attempt <= 2) {
...
describe('AgentExecutor cancellation', () => {
  test('AbortSignal 取消时 FakeTransport 收到 abort signal', async () => {
    ...
    transportWithSignal.connect = async function (agentId: string) {
      const session = await origConnect(agentId);
      return {
        execute: async (req: AgentRequest) => {
