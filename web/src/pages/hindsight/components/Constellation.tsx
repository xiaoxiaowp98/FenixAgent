...
export function Constellation({
  ...
  const { preparedNodes, linksByNode, linksWithIndices } = useMemo(() => {
    ...
    for (let li = 0; li < data.links.length; li++) {
      ...
      const color =
        linkColorFn?.(link) || link.color || LINK_TYPE_COLORS[link.type || "semantic"] || DEFAULT_NODE_COLOR;
  ...
  const animate = useCallback(() => {
    ...
    ctx.font = FONT_BOLD;
    ctx.fillStyle = isDark ? "#a1a1aa" : "#52525b";
    ctx.fillText((heatLegendLabel || t("constellation.legendLinks")).toUpperCase(), 12, 36);
    const [heatLo, heatHi] = heatLegendEndpoints || [t("constellation.legendFew"), t("constellation.legendMany")];
