...
export const KnowledgeBaseInfoSchema = z.object({
  ...
  userId: z.string().describe("知识库创建者用户 ID。"),
  organizationId: z.string().nullable().describe("所属组织 ID。"),
  embeddingModel: z.string().nullable().optional().describe("嵌入模型名。"),
  parseMethod: z.string().nullable().optional().describe("解析方法。"),
  chunkMethod: z.string().nullable().optional().describe("分块方法。"),
  remoteExists: z.boolean().optional().describe("RAGFlow 端是否存在该知识库；不存在时前端卡片置灰。"),
});

/** 创建知识库请求体 */
export const CreateKnowledgeBaseRequestSchema = z.object({
  name: z.string().optional().describe("知识库名称（创建/导入时必传）。"),
  ...
  action: z.string().optional().describe("操作类型：list-unassociated / import / 空为创建。"),
  remoteId: z.string().optional().describe("远端知识库 ID（import 时必传）。"),
});
