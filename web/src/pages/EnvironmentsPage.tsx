...
export function EnvironmentsPage({
    ...
    const [envTab, setEnvTab] = useState<"environments" | "subagents">(
        "environments",
    );
    ...
    const handleFormSubmit = useCallback(async () => {
        if (!formName || !/^[a-z0-9]+(?:-[a-z0-9]+)*$/.test(formName)) {
            setFormError(
                "名称必须为 kebab-case 格式（小写字母、数字、连字符）",
            );
            return;
        }
    ...
    const getCardStatus = (env: Environment): CardStatus => {
        ...
        if (
            env.instance_status === "running" ||
            env.instance_status === "starting"
        )
            return "running";
                ...
                {envTab === "subagents" ? (
                                        ...
                                        <div className="flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                                            ...
                                            {env.instance_id && online && (
                                                <Button
                                                    ...
                                                    title="重启实例">
                                                    <RotateCw
                                                        className={`h-3.5 w-3.5 ${refreshingEnvId === env.id ? "animate-spin" : ""}`}
                                                    />
                                                </Button>
                                            )}
                                            {env.instance_id && online && (
                    ...
                    <div className="grid grid-cols-[repeat(auto-fill,minmax(260px,1fr))] gap-3">
                        {envs.map((env) => {
                            ...
                            const instances = instancesMap[env.id] ?? [];
                            const instanceCount =
                                env.instances_count ?? instances.length;
                            const entering = enteringEnvId === env.id;
                            const online = isOnline(env);
                            ...
                            return (
                                <div
                                    ...
                                    <div className="flex items-center justify-between p-4 pb-0 pl-5">
                                        ...
                                        <span
                                            className={`shrink-0 rounded-full px-2 py-0.5 text-[11px] font-medium ${statusColors.pill}`}>
                                            {statusColors.label}
                                        </span>
                                    </div>
                                    ...
                                    <div className="px-4 pt-3 pb-2 pl-5">
                                        ...
                                        {/* Description */}
                                        {env.description && (
                                            <p className="mt-2 line-clamp-2 text-[11px] text-text-dim">
                                                {env.description}
                                            </p>
                                        )}
                                    </div>
                                            ...
                                            <div className="flex items-center gap-0.5 opacity-0 group-hover:opacity-100 transition-opacity mr-1">
                                                ...
                                                {status === "error" && (
                                                    <Button
                                                        ...
                                                        title="重试">
                                                        <RefreshCw
                                                            className={`h-3 w-3 ${refreshingEnvId === env.id ? "animate-spin" : ""}`}
                                                        />
                                                    </Button>
                                                )}
                                            </div>
                        ...
                        <div className="grid gap-4 py-4">
                            ...
                            <div className="grid gap-2">
                                ...
                                <Input
                                    ...
                                    onChange={(e) =>
                                        setFormWorkspacePath(e.target.value)
                                    }
                            ...
                            <button
                                ...
                                onClick={() =>
                                    setFormAutoStart(!formAutoStart)
                                }>
                                <span
                                ...
                                <span className="text-sm text-text-primary cursor-pointer">
                                    服务器启动时自动运行
                                </span>
                            </button>
                        </div>
                        {formError && (
                            <p className="text-sm text-status-error px-1">
                                {formError}
                            </p>
                        )}
                        <DialogFooter>
